...
if __name__ == "__main__":
    dataframe_scaled = pd.read_csv('../data/sinewave.csv')  # 读取股票文件

    dataframe = dataframe_scaled.iloc[-30:, :]

    # 归一化数据训练出来的模型
    forecast_scaled = forecast_scaled()
    inv_y_predict_1 = forecast_scaled.forecast(dataframe, dataframe_scaled, "../save_models/03122021-230302-e50.h5")
    plt = plot_results(inv_y_predict_1, dataframe_scaled.iloc[-30:, -1:].values)
    plt.savefig("inv_y_predict_1.jpg")

    # 没有归一化数据训练的模型
    forecast = forecast()
    inv_y_predict_2 = forecast.forecast(dataframe, "../save_models/04122021-165250-e50.h5")
